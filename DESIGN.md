...
| Currency globe | `src/globe.js` + `src/landdots.js` + `.orb` | Dot cloud built from **real Natural Earth land polygons** (`world-atlas` land-110m rasterized to a 2.2 degree grid, lazy chunk); the selected currency's countries light up violet and the globe turns toward them. List shows code, name and symbol; scrollable with the wheel (`data-lenis-prevent`). Map is drawn unmirrored (east to the right). The dialog animates open (panel rises + scales in, backdrop fades) and closes through the same motion, including Esc; reduced motion snaps |
